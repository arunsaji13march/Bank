    ...
    db.query("select * from login where accno=? and pass=?",[accno,pass],(error,result)=>{
        if(error){
            ...
            db.query("select * from user where accno=?",[accno],(error,results1)=>{
                ...
                res.render("../views/second",{accno: results1[0].accno,name: results1[0].name,dob: results1[0].dob,email: results1[0].email,phone: results1[0].phone,address: results1[0].address});
